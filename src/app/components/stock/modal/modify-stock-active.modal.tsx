...
import { getStockById, updateActiveStock } from "../../../services/stock.service";
...
export function ModifyStockActiveModal({ onClose, onCloseAndReload, stockId }: Props) {
  ...
  const [stockInput, setStockInput] = useState<UpdateActiveStock>({} as UpdateActiveStock);  
  ...
  useEffect(()=>{
    ...
    getStockTicks().then(value => setStockReferenceCollection(value)).catch(err => setErrorModal({ isOpen: true, msg: err }));
      ...
      {
          const stockInputAux: UpdateActiveStock =
          {
              amount: x.amount,
  ...
  function update(){      
      updateActiveStock(stockId, stockInput).then(value=>onCloseAndReload()).catch(err=>setErrorModal({isOpen:true,msg:err}));
  }
